...
export async function GET(req, { params }) {
  try {
    ...
    // If claim exists in DB, check access
    ...
    try {
      ...
      if (result.id.toString() !== '0') {
         // If it exists on-chain but not in DB, restrict to AUDITOR as a safe default for verification
         if (!dbClaim && user.role !== 'AUDITOR') {
            return NextResponse.json({ error: 'Unauthorized: Database record missing for verification' }, { status: 403 });
         }
