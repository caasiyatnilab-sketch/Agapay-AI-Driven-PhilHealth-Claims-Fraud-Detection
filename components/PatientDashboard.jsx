...
export default function PatientDashboard() {
  ...
  const highRiskCount = claims.filter((claim) => claim.riskLevel === 'HIGH' || claim.riskScore >= 0.7).length;
    ...
    <div>
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">{claims.length} total claims · {highRiskCount} high-risk screenings</p>
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            {filteredClaims.length === 0 && (
              <tr><td colSpan="7" className="px-6 py-4 text-center text-sm text-gray-500">No claims found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
