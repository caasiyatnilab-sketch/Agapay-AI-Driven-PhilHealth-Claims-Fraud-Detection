...
export default function PatientDashboard() {
  ...
  const handleSimulateOCR = async () => {
    ...
    try {
      // Add artificial delay to improve perceived UX of "AI processing"
          ...
          <form onSubmit={submitClaim} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="col-span-1 md:col-span-2 mt-2">
              ...
              <div
                role="button"
                tabIndex="0"
                aria-label="Simulate medical document OCR extraction"
                onKeyDown={handleKeyDown}
                className={`border-2 border-dashed border-gray-300 rounded-lg p-6 text-center transition-colors focus-visible:ring-2 focus-visible:ring-phBlue outline-none ${isExtracting ? 'bg-gray-100 cursor-not-allowed opacity-75' : 'cursor-pointer hover:bg-gray-50'}`}
                onClick={handleSimulateOCR}
              >
                 <div className="flex flex-col items-center justify-center">
                    {isExtracting ? (
                      <>
                        <Loader2 className="h-8 w-8 text-phBlue animate-spin mb-2" />
                        <p className="text-sm font-medium text-phBlue">Extracting data from documents...</p>
                      </>
                    ) : (
